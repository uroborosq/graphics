...
#include <cmath>
std::vector<float> &
FloydSteinbergDithering::proceed(std::vector<float> &pixels, int width, int depth, bool isColorful) {
    ...
    for (std::size_t i = 0; i < height ; i++)
        for (std::size_t j = 0; j < width; j++)
            for (int color = 0; color < numberOfColorChannels; color++)
            {
                ...
                if (j + 1 < width) {
                    pixels[(i) * width * numberOfColorChannels + (j + 1) * numberOfColorChannels + color] += error * 7.0 / 16.0;
                    if (i + 1 < height) {
                        pixels[(i + 1) * width * numberOfColorChannels + (j + 1) * numberOfColorChannels + color] += error / 16.0;
                    }
                }
                if (i + 1 < height) {
                    pixels[(i + 1) * width * numberOfColorChannels + (j) * numberOfColorChannels + color] +=
                            error * 5.0 / 16.0;
                    if (j > 0)
                        pixels[(i + 1) * width * numberOfColorChannels + (j - 1) * numberOfColorChannels + color] += error * 3.0  / 16.0;
